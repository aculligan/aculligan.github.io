...
if(tool=="doubleZoom"){
	...
	points.left=dim.left;
	points.top=dim.top;
	$(document).on("mousemove touchmove", onDoubleZoom);
}
...
var onZoom = function(e) {	
	var dim=coords(e,"#sketch");
	transf.left-=(dim.left-points.left);
	transf.top-=(dim.top-points.top);
	...
	$(".mainWrapper").css('transform', 'scale(1.5)').css('transform-origin', (transf.left) + 'px ' + (transf.top) + 'px');	
};

////////////Double Zoom - START///////////////

var onDoubleZoom = function(e) {	
	var dim=coords(e,"#sketch");
	transf.left-=(dim.left-points.left);
	transf.top-=(dim.top-points.top);
	transf.left = (transf.left < 0 ? 0 : transf.left);
	...
	transf.left = (transf.left > 1280 ? 1280 : transf.left);
	transf.top = (transf.top > 910 ? 910 : transf.top);
	$(".mainWrapper").css('transform', 'scale(2)').css('transform-origin', (transf.left) + 'px ' + (transf.top) + 'px');
...
$(document).on("mouseup touchend",function(e) {
	...
	////////////Double Zoom - START///////////////
	if(tool=='doubleZoom'){		
		$(document).off("mousemove touchmove", onDoubleZoom);
	}
	////////////Double Zoom - END///////////////
...
$(document).on("click", "[data-tool]", function () {
	...
	if(tool=='zoom'){		
		$(".mainWrapper").css('transform', 'scale(1.5)');		
	}	
	////////////Double Zoom - START///////////////
	else if(tool=='doubleZoom'){		
		$(".mainWrapper").css('transform', 'scale(2)');		
	}
	////////////Double Zoom - END///////////////
...
$(document).on("click", ".leftTool", function () {
	$(this).parent().toggleClass("enabledTool");
	if(toolLock==0||toolLock==1){
		if(toolLock==1){toolLock=0}
		else{toolLock=1;}
	
	// console.log(this);
	
	$(this).closest(".toolsWrapper").find(".toolBox").toggleClass("enabled");
	$("#sketch").toggleClass("enabled");	
	$("body").toggleClass("toolsEnabled");
	$(".blockPanel").fadeToggle();
	clearZoom();
	}
	else{//fix
		if(toolLock==2){toolLock=1;}
		
		$(this).closest(".toolsWrapper").find(".toolBox").toggleClass("enabled");
		$(".rightTool").closest(".toolsWrapper").find(".toolBox").toggleClass("enabled");
		$(".rightTool").closest(".toolsWrapper").removeClass("enabledTool");
	}

});

$(document).on("click", ".rightTool", function () {
	$(this).parent().toggleClass("enabledTool");
	if(toolLock==0||toolLock==2){
		if(toolLock==2){toolLock=0}
		else{toolLock=2;}
	...
	clearZoom();}
		...
		if(toolLock==1){toolLock=2;}
		
		$(this).closest(".toolsWrapper").find(".toolBox").toggleClass("enabled");
		$(".leftTool").closest(".toolsWrapper").find(".toolBox").toggleClass("enabled");
		$(".leftTool").closest(".toolsWrapper").removeClass("enabledTool");
...
$(document).on("click", "[data-eraser],[data-clear],[data-tool],[data-color],[data-stroke],[data-zoom],[data-unzoom],[data-doubleZoom],.highlighter,[data-hide]", function (e) {
	e.stopPropagation();	
