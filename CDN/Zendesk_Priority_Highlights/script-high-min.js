'use strict';
var removeHigh = function() {
        if (0 < window.location.href.indexOf('agent/filters')) {
            var b = $('.high-ticket-priority');
            b.removeClass('high-ticket-priority')
        }
    },
    high = function() {
        if (0 < window.location.href.indexOf('agent/filters')) {
            ...
            d.each(function() {
                ...
                var j = i.eq(c),
                    k = j.text();
                'High' == k && e.addClass('high-ticket-priority')
            })
        }
    };
$(document).ready(function() {
    removeHigh(), setTimeout(high, 1700)
}), $('*').click(function() {
    removeHigh(), setTimeout(high, 1300)
